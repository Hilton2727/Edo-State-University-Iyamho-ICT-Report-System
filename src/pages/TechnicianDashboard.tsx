...
import { fetchTechTicketStats, updateTicketStatus } from "@/services/api.service";
  ...
  useEffect(() => {
    const fetchTicketsAndStats = async () => {
      try {
        ...
        const assignedTickets = (data.tickets || []).filter(
          (ticket: any) => ticket.assignedTo && ticket.assignedTo.email === user.email
        ).map((ticket: any) => ({
          ...ticket,
          assignedTo: ticket.assignedTo ? {
            ...ticket.assignedTo,
            role: normalizeUserRole(ticket.assignedTo.role)
          } : null
        }));
        setTickets(assignedTickets);
        const ticketStats = await fetchTechTicketStats(user.id);
        setStats(ticketStats);
      } catch (error) {
        console.error("Error fetching tickets or stats:", error);
        toast.error("Failed to load tickets or stats");
      } finally {
        setLoading(false);
      }
    };
          ...
          prevTickets.map((ticket) =>
            ticket.id === ticketId
              ? {
                  ...ticket,
                  status: "In Progress",
                  assignedTo: { ...user, role: normalizeUserRole(user.role) }
                }
              : ticket
          )
        );
